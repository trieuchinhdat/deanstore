...
export default async function Page({ params }: Props) {
	...
	if (!product) notFound()
	return <Modules modules={product.modules} product={product} />
}
...
export async function generateStaticParams() {
	const slugs = await client.fetch<string[]>(
		groq`*[_type == 'product.detail' && defined(metadata.slug.current)].metadata.slug.current`,
	)
...
async function getProduct(params: Params) {
	const productTemplateExists = await fetchSanityLive<boolean>({
		query: groq`count(*[_type == 'global-module' && path == '${PRODUCT_DIR}/']) > 0`,
	})

	if (!productTemplateExists) throw new Error(errors.missingProductTemplate)

	const { slug, lang } = processSlug(params)

	return await fetchSanityLive<
		Sanity.ProductDetail & { modules: Sanity.Module[] }
	>({
		query: groq`*[
